...
set "sourcePersist=C:\Projekte\xml2OH\export\persistence"
...
set "targetPersist=\\galactus\openHAB-conf\persistence"
...
net use %targetPersist% /user:%username% %password% >nul 2>&1
...
REM Dateien aus dem "things"-Ordner kopieren
echo Kopiere Dateien aus "%sourcePersist%" nach "%targetPersist%"...
xcopy "%sourcePersist%\*" "%targetPersist%\" /E /Y /Q
if %ERRORLEVEL% equ 0 (
    echo Persistence-Dateien erfolgreich kopiert!
) else (
    echo FEHLER: Beim Kopieren der Persistence-Dateien ist ein Fehler aufgetreten!
)
...
net use %targetPersist% /delete >nul 2>&1
